...
void printMatrix(float*, unsigned int);
...
// Kernel
__global__
void heatDistKernel(float* d_playground, float* d_temp, unsigned int upper)
{
  unsigned int Row = blockIdx.y * blockDim.y + threadIdx.y;
  unsigned int Col = blockIdx.x * blockDim.x + threadIdx.x;

  if ((Row < upper) && (Col < upper) && (Row > 0) && (Col > 0))
  {
    d_temp[index(Row, Col, upper + 1)] = (d_playground[index(Row - 1, Col, upper + 1)] +
                               d_playground[index(Row + 1, Col, upper + 1)] +
                               d_playground[index(Row, Col - 1, upper + 1)] +
                               d_playground[index(Row, Col + 1, upper + 1)] ) / 4.0;
...
// Print
void printMatrix(float* playground,unsigned int N)
{
...
void  gpu_heat_dist(float * playground, unsigned int N, unsigned int iterations)
{
  ...
  float *d_playground, *d_temp, *result;

  // Made use of 2d blocks and threads as it will make it easier to work on data
  // represented as 2d
  dim3 numBlocks(ceil(N/TILE), ceil(N/TILE), 1);
  ...
  // Step 2 : Launch Kernel for given iteration times
  for (unsigned int i = 0; i < iterations; i++)
  {
    heatDistKernel<<<numBlocks, threadsPerBlock>>>(d_playground, d_temp, N - 1);
    result = d_temp;
    // Swap both the pointers
    float* temp = d_playground;
    d_playground = d_temp;
    d_temp = temp;
  }

  // Step 3 : Bring result back to host
  cudaMemcpy(playground, result, size, cudaMemcpyDeviceToHost);

  // Step 4 : Free device memory
  cudaFree(d_playground);
  cudaFree(d_temp);

  //printMatrix(playground, N);
